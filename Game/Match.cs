...
namespace Game
{
    public class Match
    {
        ...
        public Board board;
        public Decks decks;
        public Match(Player p1, Player p2, Board b, Decks d)
        {
            player = p1;
            opponent = p2;
            board = b;
        ...
        public void init()
        {
            ...
            try
            {
                musicPlayer = new MusicPlayer(@"assets\music\Fight.wav");
                musicPlayer.loop();
            }
            catch (Exception e)
            {
                Console.WriteLine("Music not available: " + e.ToString());
                Utility.Debug.wait(Utility.config.debug, 2000);
            }
            ...
            while (isPlayerAlive(opponent) && isPlayerAlive(player))
            {
                    ...
                    while (isSelecting)
                    {
                        ...
                        try
                        {
                            ...
                            if (input == 1)
                            {
                                ...
                                try
                                {
                                    Card c = Deck.getCard(selectMenu.getStringValue(), decks);
                                    ...
                                    {
                                        Utility.Debug.printDebug("Player mana: " + player.getMana() + "\nCard Mana: " + c.mana + "\nPlayer has card: " + player.hasCard(c));
                                        Utility.Debug.printDebug(c.toString());
                ...
                if ((opponent.getMana() > (mana / 2)) && !boardFull(true, board))
                {
                    while ((opponent.getMana() > (mana / 2)) && !boardFull(true, board) && tries < 10)
                    {
                        ...
                        if (isValidHold(opponent, c) && isValidPlay(opponent, board, c, placement))
                        {
                            ...
                            Utility.Debug.printDebug("Played a card");
                            Utility.ConsoleFunctions.wait(1000);
                        }
                        else
                        {
                            Utility.Debug.printDebug("In the while loop");
                            tries++;
                        }
                    }
                ...
                {
                    Utility.Debug.printDebug("Didn't play a card");
                    Utility.ConsoleFunctions.wait(1000);
                }
                if (tries >= 10)
                {
                ...
                while (opponent.getMana() > 0 && !boardEmpty(true, board))
                {
                    ...
                    if (isValidAttack(opponent))
                    {
                        ...
                        draw(opponent, player, board);
                        Utility.Debug.printDebug("Attacked");
                        Utility.ConsoleFunctions.wait(3000);
                    }
                    Utility.Debug.printDebug("Didn't attack");
                    Utility.ConsoleFunctions.wait(3000);
        ...
        public Coordinate findRandomCardOnBoard(bool isEnemy /*y = 1 or 0*/, Board board)
        {
        ...
        public bool boardFull(bool isEnemy, Board board)
        {
            if (isEnemy)
            {
        ...
        public bool boardEmpty(bool isEnemy, Board board)
        {
            if (isEnemy)
            {
                for (int i = 1; i < board.getCards()[0].Length; i++)
        ...
        public void draw(Player p1, Player p2, Board board)
        {
        ...
        public bool isValidPlay(Player p, Board board, Card c, Coordinate pos)
        {
            return pos.x >= 0
                    && pos.x <= 3
        ...
        public void attack(Player p1, Player p2, Board board, Coordinate card1pos, Coordinate card2Pos)
        {
        ...
        private void removeIfDead(Board board, Coordinate card1pos, Card c)
        {
            if (c.hp <= 0)
            {
                c.hp = Deck.getCardByID((int)c.ID).hp; //this should set card back to inital hp
        ...
        public bool isValidCardSelect(Board board, Coordinate selectedPos)
        {
            return board.cardExists(selectedPos);
        }

        //mana check
        public bool isValidAttack(Player p)
        {
            return p.getMana() >= 1;
        }
         ...
         * @return
         */
        public bool isValidHold(Player p, Card c)
        {
